...
custom mythos CodeQL memory queries, CSA/native bundle
results by nearby file/line, and `llm/retrieval-plan.json/.md`, which points the LLM at risky
files, symbol-index candidate callers, semantic-index queries, CodeQL follow-up commands, and
source-search patterns. The
LLM should start from those files, not from raw source.

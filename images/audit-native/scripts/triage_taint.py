#!/usr/bin/env python3
"""triage_taint.py — summarize svf-taint.json: counts by kind / sink file / base object,
...
a = ap.parse_args()
r = json.load(open(a.path)); F = r["findings"]
def sinkfile(f):
...
[print(f"  {n:6d}  {k}") for k, n in Counter(f["base_object"] for f in F if f["kind"] == "TAINTED_INDEX").most_common(a.top)]
rx = re.compile(a.files); hits = [f for f in F if rx.search(f["sink_loc"]) or rx.search(f["sink_function"])]; seen = set()
print(f"\ncandidates in files matching /{a.files}/: {len(hits)}")
for f in hits:
    key = (f["kind"], f["sink_loc"])
    if key in seen: continue
    seen.add(key)
    print(f"  {f['kind']:20s} {sinkfile(f)}  {f['sink_loc'][:70]}\n{'':24s}fn={f['sink_function'][:60]}  base={f['base_kind']}:{f['base_object'][:40]} {f['base_bytes']}B/{f['base_elements']}el  src={f['source']}@{f['source_function'][:30]}")

...
class CancellationTests(DispatchCase):
    ...
    def interrupting_wait(self):
        # C02 HOLDS an interrupt raised inside its resumable wait: the pool is canceled, the wait is
        # entered again, the manifest is published and the interrupt re-raised afterwards. The
        # wrapper raises exactly one, in the coordinator, the first time the wait runs.
        real, raised = pr._wait, []
...
class LowerLayerRefusalTests(DispatchCase):
    ...
    def test_any_exception_from_the_adapter_verifier_is_not_assessed(self):
        # The adapter's verifier runs inside C02's reader (a consumer calls none itself). An
        # exception there is a refusal: the instance no longer derives ``succeeded``, so the
        # published manifest no longer verifies and no cell of that pool is assessed.
        pointer = self.dispatch()
        plan, specifications = self.plan(), self.specifications(pointer)
        with mock.patch.object(pi, "load_verified_result", side_effect=TypeError(MARKER)):
        ...
        self.assertEqual({cell["not_assessed_reason"] for cell in accounting["cells"]}, {"pool_unverifiable"})
        self.assertEqual({pool["manifest_verified"] for pool in accounting["pools"]}, {False})
        self.assertEqual(set(dispositions(accounting)), {"not_assessed"})
        self.assertNotIn(MARKER, "".join(every_text(accounting)))
    ...
    def rows(self, static_valid: bool) -> list:
        ...
        for batch in manifest["batches"]:
            ...
            entries.append({"valid_result": valid, "not_assessed_reason": None if valid or dynamic else "cell_state",
                            "instance_id": None, "state": None, "state_reason": None, "adapter_status": None,
                            "adapter_cause": None,

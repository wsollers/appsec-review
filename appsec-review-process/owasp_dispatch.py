...
def pool_context(plan: Plan, facts: DispatchFacts, attempt: Path, wave: int) -> ps.PoolContext:
    return ps.PoolContext(
        ...
        # launched or verified from this context, which is the one case PoolContext allows None.
...
def _cell_candidate(plan: Plan, attempt: Path, wave: int, pool_directory: str, item: pr.VerifiedInstance,
                    context: ps.PoolContext) -> tuple:
    """(candidate, reason) for a ``succeeded`` instance. The adapter's own verifier decides first:
    ``item.result`` is what B14's verifier returned during C02's verification of this very manifest
    (a consumer calls no adapter verifier itself), and the result file on disk must still be the
    bytes the record pins. The candidate is read once, and those bytes are what is hashed, parsed
    and bound to this instance."""
    entry, recorded = item.instance.entry, item.record
    ...
    try:
        ...
        if (result is None or result["execution_status"] != "OK" or recorded["result_file"] is None
    ...
    path = instance_root.joinpath(*item.instance.request.request["output_root"].split("/")) / CANDIDATE_FILE
    try:
    ...
    for cell, entry in zip(cells, entries):
        ...
        for position, (fragment_id, assignment_id, authority) in enumerate(cell.fragments):
            ...
            by_fragment.setdefault(assignment_id, []).append({
                ...
                "instance_id": entry["instance_id"], "state": entry["state"], "state_reason": entry["state_reason"],
...
def derive_accounting(plan: Plan, facts: DispatchFacts, *, attempt: Path, attempt_id: str,
    ...
    manifests (read through C02's reader, which is the one reading of a pool that tolerates an
    honestly recorded ``missing`` instance), the adapter results that verification obtained, the
    cells' candidate bytes and T07's own files. It never raises for something a lower layer
    refuses: that is ``not_assessed``."""
    pools, observed = [], {}
    for wave, spec in enumerate(specifications):
        ...
        try:
            verified = pr.load_verified_manifest(
            ...
            by_group = {item.record["group_id"]: item for item in verified.instances}
            if (len(by_group) != len(manifest["instances"]) or any(item.record["ordinal"] != 0 for item in by_group.values())
        ...
        for index in plan.waves[wave]:
            ...
            if manifest is None:
                ...
                continue
            item = by_group[cell.group_id]
            candidate, reason = None, "cell_state"
            if item.state == pr.SUCCEEDED:
                candidate, reason = _cell_candidate(plan, attempt, wave, directory, item, context)
            observed[index] = (item.record, candidate, reason)
    ...
    for index, cell in enumerate(plan.cells):
        ...
        entry = {
            ...
            "state": state, "state_reason": record["state_reason"] if record else None,
            "adapter_status": record["adapter_status"] if record else None,
...
def _run_wave(plan: Plan, runtime: DispatchRuntime, attempt: Path, wave: int, spec: dict) -> BaseException | None:
    ...
    try:
        ...
        # C02 builds the B14 runtime itself from the context's facts and these launch-only objects,
        # so what runs is what the same context later verifies.
        pr.run_rendezvous(pool_root, expected_spec=spec, context=context, runtime=pr.RendezvousRuntime(
            rendezvous_parent=rendezvous_parent, invoker=runtime.invoker, clock=runtime.clock,
            stop_grace_seconds=runtime.stop_grace_seconds, cancel=cancel, max_parallel=runtime.max_parallel,
            wait_limit_seconds=runtime.wait_limit_seconds, drain_seconds=runtime.drain_seconds))

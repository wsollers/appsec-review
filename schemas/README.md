...
- `owasp-dispatch-cell.schema.json` -- one expected cell (one T06 handoff): its ordinal, member hash and identifier digests, mode and disposition (`dispatched` or `request_only_not_dispatched`), wave, group and instance, the C02 state with adapter status and cause, `terminal_class` (`valid_result`, `failed`, `canceled`, `timed_out`, `skipped`, `invalid`, `request_only`), the candidate file record, the T07 validation outcome (`accepted`, `refused`, `unavailable`, `not_submitted`) with T07's attempt, status and hashes, `valid_result` and a closed `not_assessed_reason`.
- `owasp-dispatch-row.schema.json` -- one T05 worklist row, exactly once: its index, assignment digest and source row hash, applicability and worklist disposition, `join_required`, each fragment with its cell, authority, disposition (`deferred_to_validated_result`, `not_assessed`, `request_only`) and provenance, the row disposition, and `final_control_status_issued`, which is only `false`.

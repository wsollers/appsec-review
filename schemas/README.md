...
- `pool-expansion.schema.json` -- `expansion.json`, written last into a pool root (`appsec-review/pool-expansion/1.0`): specification hash and file record, the id derivation id, the derived pool directory, the pool identity, `state` (`EMPTY` or `POPULATED`) with the reason, typed `groups`, the ordered `instances`, recorded `totals` (per kind, per resource pool, persona slot request, persona unit sums, timeout sum and maximum) and `expansion_sha256`. No timestamp and no host path; a group's `identity_sha256` is its adapter identity at every count, zero included. The specification hash, the pool directory and every instance id are the same on any host; four request-derived hashes of a pinned-container instance (`request_file`, `request_sha256`, `adapter_fingerprint_sha256`, `input_fingerprint`) cover B13's resolved absolute `host_path` and are bound to the host's mount roots.
- `pool-expansion-instance.schema.json` -- one expected instance: deterministic `instance_id` (also its `attempt_id`; bare 32-digit hex, because the V06 redactor treats `prefix-<hex>` as a secret, which is also why the B13 container name is not recorded), group and ordinal, worker kind, private `attempt_root`, the adapter `request_schema`, `request_file` `{path, sha256, bytes}` and `request_sha256`, constant `writable_paths` and `log_path`, the derived `resource_pool`, `granted_permission_kinds`, `memory_heavy`, `persona_slot_request` (personas), `timeout_seconds`, and the permission, adapter and instance input fingerprints.

Count, total and budget bounds, group order, the permission re-derivation, image resolution, the adapters' own request validation per instance, the same validation of a zero-count group's template, target-mount resolution (`resolve_target_mounts`), the pool-parent privacy boundary (device+inode), declared mount roots, collision checks, pool derivation through `resource_pools.derive_pool`, and the on-disk re-derivation of every file are `pool_specification.py` responsibilities (`spec_errors`, `plan_expansion`, `expand_pool`, `check_expansion` / `verify_expansion`), not expressible here. Verifier findings carry stable codes (`FINDING_CODES`); only a pinned-container `requests/<id>.json` (not a schema of this batch: it is B13's request) can hold a host path. `plan_expansion` is the one rule the expander and the verifier share. `expansion_sha256` is an integrity check, not an authenticator.
...
That the instance list is the expansion's (each once, in order), the state of each instance (through the adapter's own on-disk verifier, never a returned value), which fields and which reasons a state carries (`REASONS_BY_STATE`), the counts, the outcome, `wait_all` being the JSON value `true`, unique JSON keys, the rendezvous location, exclusive publication and the byte-for-byte re-derivation are `pool_rendezvous.py` responsibilities (`classify_instance`, `pool_outcome`, `manifest_errors`, `verify_manifest`), not expressible here. `classify_instance` is the one rule the coordinator and the verifier share. `manifest_sha256` is an integrity check, not an authenticator.

OWASP validator dispatch, wait-all and failure accounting (G02 task T10; a standalone worker on the
B13-C02 pool foundations -- not registered in Dagster, the lifecycle graph, the registry or the
parity manifest, and without a model client, see `docs/proposals/owasp-workbench/task-series.md`):

- `owasp-dispatch-request.schema.json` -- `runs/<run_id>/inputs/owasp-dispatch-request.json`: the run, one exact T06 publication (`attempt_id`, accepted pointer path and hash, handoff set path and hash), the tracked dispatch configuration path and digest, and one exact `model` (`$ref` to the B14 model identity). No property for a tool, a permission, a claim class, a budget, a persona or a path a cell may write.
- `owasp-dispatch-config.schema.json` -- the tracked versioned configuration under `appsec-review-process/config/owasp-dispatch/`: the job template and persona the cells run as, the claim class a cell must be allowed to issue, the classes a cell may be narrowed to, the T06 budget name to C01 `budget_class` map, the B14 budget limits and timeout ceiling per budget name, the T06-prohibited-claim to registry-class map, the pool bounds, and `dynamic_execution`, `manual_observation` and `network_access`, which are only `false`.
- `owasp-dispatch-attempt.schema.json` and `owasp-dispatch-status.schema.json` -- the immutable allocation record of an attempt (its input fingerprint and the one decision instant its permission decisions use) and its lifecycle status with a closed refusal code. Neither has free text; the status has no timestamp.
- `owasp-dispatch-accounting.schema.json` -- `outputs/owasp-dispatch-accounting.json`: the accounting rule id, run/job/attempt identity and input fingerprint, the pinned T06 publication, worklist and configuration, the composition hash with persona, template, model and invoker, the cell boundary (claim classes, an empty tool list, no granted permission kind, four capabilities that are only `false`), a claims boundary of seven values that are only `false`, the explicit vocabulary map, `pools`, the dispatch outcome, `degraded`, `status` (`OK` or `OK_WITH_GAPS`), `cells`, `rows`, `counts` and `accounting_sha256`. No timestamp, no host path, no free text and no T05-T07 identifier: the V06 redactor replaces their `prefix-<hex>` shape, so they are published as `id_digest` values and ordinals.
- `owasp-dispatch-pool.schema.json` -- one wave: its C01 pool directory and specification hash, whether its C02 manifest verified, and then the expansion state, the empty-pool reason, the outcome and the manifest hash.
- `owasp-dispatch-cell.schema.json` -- one expected cell (one T06 handoff): its ordinal, member hash and identifier digests, mode and disposition (`dispatched` or `request_only_not_dispatched`), wave, group and instance, the C02 state with its closed `state_reason` (C02's `STATE_REASONS`) and the adapter status and cause, `terminal_class` (`valid_result`, `failed`, `canceled`, `timed_out`, `skipped`, `invalid`, `request_only`), the candidate file record, the T07 validation outcome (`accepted`, `refused`, `unavailable`, `not_submitted`) with T07's attempt, status and hashes, `valid_result` and a closed `not_assessed_reason`.
- `owasp-dispatch-row.schema.json` -- one T05 worklist row, exactly once: its index, assignment digest and source row hash, applicability and worklist disposition, `join_required`, each fragment with its cell, authority, disposition (`deferred_to_validated_result`, `not_assessed`, `request_only`) and provenance (the instance, its C02 state and `state_reason`, adapter status and cause, and the T07 attempt and result hash), the row disposition, and `final_control_status_issued`, which is only `false`.

The exact newest accepted T06 lineage, worklist coverage, the registry composition and claim ceiling, the prompt and evidence pins, that the listed booleans are JSON `false` (the subset's `const` also admits `0`), which handoffs are dispatched, the candidate's bytes, single reading and producer identity, T07's verdict read from T07's own files, the row rule, newest-attempt blocking, reuse and the byte-for-byte re-derivation of every T10 file are `owasp_dispatch.py` responsibilities (`load_plan`, `build_specifications`, `derive_accounting`, `verify_publication`, `load_verified_accounting`), not expressible here. `derive_accounting` is the one rule the producer and the verifier share. `accounting_sha256` is an integrity check, not an authenticator.

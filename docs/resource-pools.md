...
- Pool lanes are outside these pools for now. `pool_rendezvous.run_rendezvous` (C02) launches its
  ...
  persona invocations claim no slot here and its own caps see only their own process. **Owner
  decision 2026-09-21: one engagement at a time for now**, so at most one rendezvous runs on the host
  and those caps are the host-wide ones. The job whose op calls `run_rendezvous` (T10) must carry a
  run-level tag with a tag concurrency limit of 1; the target design launches instances as
  dynamically mapped pooled ops ([pool rendezvous](pool-rendezvous.md), "Constraint: one rendezvous
  at a time").

...
For new Dagster-orchestrated runs, generated intelligence belongs under the owning run's
`appsec-review-process/runs/<run_id>/data/jobs/.../attempts/<attempt_id>/` directory and is
published through that job's `accepted.json`. Older examples in this document use
`scratch/<project>-engagement/` paths to describe the legacy evidence shape; treat those as
compatibility examples unless the evidence has been explicitly imported into run-owned `data/`.
...
For Dagster jobs, allocate `--out` and manifest paths under the job attempt directory. The
following command shape preserves the legacy scratch naming for compatibility examples.
...
For Dagster jobs, write parsed intelligence under the owning attempt directory and publish through
the job output contract. The path below is the legacy scratch-compatible form.

...
flowchart TD
  ...
  S2b[2b. Discovery gates: repository partition map, then developer, devops and SRE operations-topology discovery - supplied, validated records] --> G1{native code in scope?}
...
supplied, schema- and freshness-validated records (`discovery_gate.py`); they do not perform the
analysis themselves. SRE topology chains after the accepted devops record instead of the partition
map directly (2026-09-24): it reads the containers/services devops discovery already found.

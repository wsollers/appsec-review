...
step_meta() {
  ...
  case "$step" in
    ...
    iac-k8s) SUBDIR=iac-k8s; ALLOW_NONZERO=1; EXPECTED=iac-k8s/kube-linter.json; CMD=(bash -lc "kube-linter lint /workspace --format json > /evidence/iac-k8s/kube-linter.json || true");;
    dockerfile-lint) SUBDIR=iac-docker; ALLOW_NONZERO=1; CMD=(bash /opt/scripts/run-dockerfile-lint.sh);;
    docker-base-images) SUBDIR=iac-docker; ALLOW_NONZERO=1; CMD=(bash -lc "find /workspace -iname 'Dockerfile*' -type f -print0 | xargs -0 -r awk 'BEGIN{IGNORECASE=1} /^FROM[[:space:]]+/ {print FILENAME \":\" NR \":\" \$0}' > /evidence/iac-docker/base-images.txt");;
